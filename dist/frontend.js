...
function hasTavernHelperScripts(char) {
...
// src/render/chat-variable-state.ts
function record(value) {
  return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}
function chatVariableState(chatId, metadata) {
...
function bindChatVariableState(frame, ctx, initial, followsActiveChat) {
  ...
  if (!set)
    listeners.set(ctx, set = new Set);
  set.add(accept);
  const changed = ctx.events.on("CHAT_CHANGED", (data) => {
    ...
    if (id !== state.chatId)
      return;
    if (chat.metadata)
      accept(chatVariableState(state.chatId, chat.metadata));
    else
      refresh();
  });
  const switchChat = () => {
    if (!followsActiveChat)
      return;
  ...
  const settings = ctx.events.on("SETTINGS_UPDATED", (data) => {
    if (record(data).key === "activeChatId")
      switchChat();
  });
...
var DISPATCH_PREFIX_RE = /^\s*\/(setvar|setchatvar|setgvar|setglobalvar|sys|inject|flushinject|listinjects)\b/i;
...
// src/render/chat-variables-shim.ts
function chatVariablesShim() {
  return `
...
function dispatchThRequest(frame, request, context, ctx) {
  ...
  if (op === "th-replace-chat-variables" && (body.chatId !== context.chatId || typeof ctx.getActiveChat === "function" && ctx.getActiveChat().chatId !== context.chatId)) {
    respond({ ok: false, error: "The chat changed before the variable update was dispatched" });
    return;
  }
  unsub = ctx.onBackendMessage((payload) => {
    if (!isThHelpersResponse(payload, backendRequestId))
      return;
    if (payload.ok && op === "th-replace-chat-variables") {
      publishChatVariableState(ctx, payload.result);
    }
...
var jquery_3_5_1_min_default = `/*! jQuery v3.5.1 | (c) JS Foundation and other contributors | jquery.org/license */
...
async function buildWidgetIframe(html, scriptName, scriptId, messageId, ctx) {
  ...
  iframe.style.setProperty("clip-path", "none", "important");
...
function containsDocumentShell(html) {
  return /<!doctype\s+html\b|<html\b|<head\b|<body\b/i.test(html);
}
function widgetNeedsIsolation(html) {
  return containsScriptTag(html) || containsInlineEventHandler(html) || containsDocumentShell(html);
}
...
function buildHeadInjection(iframeCtx) {
  ...
  const thHelpers = shouldInjectThHelpersShim(iframeCtx.env) ? thHelpersShim({
    ...
    variablesSnapshot: iframeCtx.chatState?.allVariables,
...
var LEGACY_WIDGET_FALLBACK_GAP_PX = 6;
function applyAdaptiveWidgetSpacing(iframe, payload) {
  let ownsVerticalSpacing = false;
  if (payload && typeof payload === "object") {
    ownsVerticalSpacing = payload.ownsVerticalSpacing === true;
  }
  const gap = ownsVerticalSpacing ? 0 : LEGACY_WIDGET_FALLBACK_GAP_PX;
  iframe.style.marginTop = gap + "px";
  iframe.style.marginRight = "0";
  iframe.style.marginBottom = gap + "px";
  iframe.style.marginLeft = "0";
  iframe.setAttribute("data-vishrun-own-spacing", ownsVerticalSpacing ? "1" : "0");
}
function routeChildMessage(frame, payload, ctx, iframeCtx) {
  if (!payload || typeof payload !== "object")
    return;
  const p = payload;
  if (p.kind === "vishrun-layout-metrics") {
    applyAdaptiveWidgetSpacing(frame.element, p.payload);
  } else if (p.kind === "set-chat-messages") {
...
var inFlightMessages = new Set;
var pendingRescans = new Map;
...
async function processNode(root, scripts, ctx) {
  ...
  try {
    ...
    if (pending) {
      ...
      Promise.resolve().then(() => processNode(pending.root, pending.scripts, pending.ctx));
    }
  }
}
  ...
  for (const tn of textNodes) {
    ...
    if (tn.parentNode === parent && parent.isConnected) {
      ...
      for (const widget of insertedWidgets) {
        if (widget.isConnected)
          cleanupEmptyAroundWidget(widget, stopAt);
      }
    } else {
      if (VSH_VISHRUN_DIAG) {
        console.log("[vishrun:render] placeholder-skipped", JSON.stringify({
    ...
    try {
      ...
      nodes.forEach((n, i) => {
        ...
        if (messageContent?.getAttribute("data-display-pending") === "true") {
          if (nodeMessageId)
            pendingMessageIds.add(nodeMessageId);
          return;
        }
      ...
      if (pendingMessageIds.size > 0) {
        processMessageIdsNow(pendingMessageIds, compiled);
      }
...
// src/settings/script-types.ts
function isFolder(tree) {
  return tree.type === "folder";
}
function makeScript(partial = {}) {
  return {
    type: "script",
...
function normalizeScriptTree(raw) {
  ...
  if (r.type === "folder") {
    return {
      ...
      scripts: Array.isArray(r.scripts) ? r.scripts.map(normalizeScriptTree).filter((s) => s !== null && s.type === "script") : []
    };
  }
  return makeScript({
    enabled: typeof r.enabled === "boolean" ? r.enabled : true,
    name: typeof r.name === "string" ? r.name : typeof r.scriptName === "string" ? r.scriptName : "",
    id: typeof r.id === "string" ? r.id : crypto.randomUUID(),
    content: typeof r.content === "string" ? r.content : "",
    info: typeof r.info === "string" ? r.info : "",
    button: r.button && typeof r.button === "object" ? r.button : { enabled: true, buttons: [] },
    data: r.data && typeof r.data === "object" ? r.data : {},
    export_with: r.export_with && typeof r.export_with === "object" ? r.export_with : { data: true, button: true }
  });
}
function normalizeScriptTrees2(raw) {
  return raw.map(normalizeScriptTree).filter((s) => s !== null);
}

// src/settings/script-storage.ts
var BASE = "/api/v1";
var GLOBAL_KEY = encodeURIComponent("vishrun_chibijokes.scripts.global");
...
class ScriptStorageClient {
  async loadGlobal() {
    const row = await getJson(`${BASE}/settings/${GLOBAL_KEY}`);
    return Array.isArray(row?.value?.scripts) ? normalizeScriptTrees2(row.value.scripts) : [];
  }
  async saveGlobal(scripts) {
    await putJson(`${BASE}/settings/${GLOBAL_KEY}`, { value: { scripts } });
  }
  async loadPreset(presetId) {
    if (!presetId)
      return [];
    const row = await getJson(`${BASE}/settings/${presetKey(presetId)}`);
    return Array.isArray(row?.value?.scripts) ? normalizeScriptTrees2(row.value.scripts) : [];
  }
  async savePreset(presetId, scripts) {
    if (!presetId)
      return;
    await putJson(`${BASE}/settings/${presetKey(presetId)}`, { value: { scripts } });
  }
  async loadCharacter(characterId) {
    const char = await getJson(`${BASE}/characters/${encodeURIComponent(characterId)}`);
    const scripts = char?.extensions?.tavern_helper?.scripts;
    return Array.isArray(scripts) ? normalizeScriptTrees2(scripts) : [];
  }
...
async function loadEnabledScripts(characterId, presetId) {
  const secOn = (t) => typeof localStorage !== "undefined" ? localStorage.getItem(`vsh_section_enabled_${t}`) !== "false" : true;
  const storage = new ScriptStorageClient;
  const [global, preset, char] = await Promise.all([
    secOn("global") ? storage.loadGlobal() : Promise.resolve([]),
    secOn("preset") ? storage.loadPreset(presetId) : Promise.resolve([]),
    secOn("character") && characterId ? storage.loadCharacter(characterId) : Promise.resolve([])
  ]);
  return [
    ...flattenEnabled(global).map((s) => ({ ...s, scope: "global" })),
    ...flattenEnabled(char).map((s) => ({ ...s, scope: "character" })),
    ...flattenEnabled(preset).map((s) => ({ ...s, scope: "preset" }))
  ];
}
async function initScriptStorage() {
  const storage = new ScriptStorageClient;
  const globalResp = await fetch(`${BASE}/settings/${GLOBAL_KEY}`, { credentials: "same-origin" });
  if (!globalResp.ok)
    await storage.saveGlobal([]);
}
async function getActiveLoomPresetId() {
  const keys = ["selectedLoomStyles", "activeLoomPresetId"];
  for (const key of keys) {
    const row = await getJson(`${BASE}/settings/${key}`);
    const id = row?.value;
...
async function getPresetName(presetId) {
  const preset = await getJson(`${BASE}/presets/${encodeURIComponent(presetId)}`);
  return typeof preset?.name === "string" ? preset.name : presetId;
}

// src/settings/scripts-panel.ts
var CSS2 = `
.vsh-panel-header {
  display: flex;
  align-items: center;
...
`;
function createScriptsPanel(root, ctx, onScriptsSaved, onReloadScript) {
  const storage = new ScriptStorageClient;
  const removeStyle = ctx.dom.addStyle(CSS2);
  let globalScripts = [];
  let charScripts = [];
  let presetScripts = [];
  ...
  const LS_KEY = (t) => `vsh_section_enabled_${t}`;
  ...
  importBtn.addEventListener("click", () => {
    ...
    fileInput.addEventListener("change", async () => {
      ...
      try {
        ...
        const trees = normalizeScriptTrees(Array.isArray(parsed) ? parsed : parsed.scripts ?? []);
  ...
  function makeSection(title, subtitle, target) {
    const el = document.createElement("div");
    el.className = "vsh-section";
    if (!sectionEnabled[target])
      el.classList.add("vsh-section-disabled");
  ...
  function getCharSubtitle() {
    const card = getActiveCard();
    if (card?.characterName)
      return `\uD83C\uDFAD Bound to: ${card.characterName}`;
    if (currentCharId)
      return "\uD83C\uDFAD Bound to current character";
    return "\uD83C\uDFAD Load a character to view character scripts";
  }
  function getPresetSubtitle() {
    if (currentPresetName)
      return `⚙️ Bound to: ${currentPresetName}`;
    if (currentPresetId)
      return `⚙️ Bound to current loom`;
    return "⚙️ Load a loom to view preset scripts";
  }
  function matchesSearch(name) {
    return !searchFilter || name.toLowerCase().includes(searchFilter);
  }
  function renderList(listEl, scripts, target) {
    listEl.innerHTML = "";
    const visible = searchFilter ? scripts.filter((t) => t.type === "folder" ? matchesSearch(t.name) || t.scripts.some((s) => matchesSearch(s.name)) : matchesSearch(t.name)) : scripts;
    if (visible.length === 0) {
    ...
    for (const tree of visible) {
      listEl.appendChild(isFolder(tree) ? buildFolderItem(tree, target) : buildScriptItem(tree, scripts, target));
    }
  }
  function renderGlobal() {
    renderList(globalSec.list, globalScripts, "global");
  }
  function renderChar() {
  ...
  function buildScriptItem(script, parentList, target) {
    ...
    item.addEventListener("drop", (e) => {
      ...
      if (destIdx === -1)
        return;
      if (dragSrc.list === parentList && dragSrc.idx === destIdx)
        return;
  ...
  function buildEditor(script, target, onNameChange) {
    const el = document.createElement("div");
    ...
    contentArea.placeholder = `// JavaScript runs in an isolated iframe.
// The tavernHelper API is available via window.tavernHelper.`;
    ...
    try {
      dataArea.value = JSON.stringify(script.data, null, 2);
    } catch {
      dataArea.value = "{}";
    }
    ...
    exportRow.append(mkCheck("Data", script.export_with.data, (v) => {
      script.export_with.data = v;
    }), mkCheck("Buttons", script.export_with.button, (v) => {
      script.export_with.button = v;
    }));
  ...
  function buildFolderItem(folder, target) {
    const item = document.createElement("div");
  ...
  return {
    onCharacterChanged(characterId) {
      ...
      charScripts = [];
      renderChar();
      storage.loadCharacter(characterId).then((scripts) => {
...
// src/settings/script-runner.ts
var EVENT_BRIDGE_SHIM = `<script>(function(){
var ET={
...
var CLIPBOARD_SHIM = `<script>(function(){
try{
...
class ScriptRunner {
  ...
  async run(scripts, chatId) {
    ...
    for (const script of scripts) {
      ...
      if (frame && (frame.contentHash !== hashContent(script.content) || frame.reloadMemo !== (this.reloadMemos.get(script.id) ?? ""))) {
        this.destroyFrame(frame);
        this.frames.delete(key);
      }
    }
    const scriptsToLaunch = [...scripts].filter((s) => !this.frames.has(frameKey(s.scope, s.id))).sort((a, b) => a.id < b.id ? -1 : a.id > b.id ? 1 : 0);
    ...
    if (effectiveChatId) {
      ...
      if ((this.ctx.getActiveChat().chatId ?? "") !== effectiveChatId)
        return;
  ...
  launchFrame(script, chatId, messagesSnapshot, variablesSnapshot, chatState) {
    try {
...
`);
  ...
  handleActiveChatSettingChanged(data) {
    const payload = data && typeof data === "object" ? data : null;
    if (payload?.key !== "activeChatId")
      return;
    const nextChatId = this.ctx.getActiveChat().chatId ?? null;
    if (nextChatId === this.activeChatId)
      return;
  ...
  async handleChatSwitched(data) {
    const payload = data && typeof data === "object" ? data : null;
    const targetChatId = typeof payload?.chatId === "string" ? payload.chatId : this.ctx.getActiveChat().chatId ?? this.activeChatId;
  ...
  async handleChatChanged(data) {
    const payload = data && typeof data === "object" ? data : null;
    const payloadChatId = typeof payload?.chat?.id === "string" ? payload.chat.id : typeof payload?.chatId === "string" ? payload.chatId : this.activeChatId;
    if (payloadChatId && this.activeChatId && payloadChatId !== this.activeChatId)
      return;
    ...
    const changedFields = Array.isArray(payload?.changedFields) ? payload.changedFields.filter((v) => typeof v === "string") : [];
    const variableChange = changedFields.length === 0 || changedFields.some((field) => field === "metadata.chat_variables" || field === "metadata.macro_variables" || field.startsWith("metadata.chat_variables.") || field.startsWith("metadata.macro_variables."));
...
function setup(ctx) {
  ...
  window.__vishrunRegisterUserMessageProcessor = (handler) => {
    if (typeof handler !== "function")
      throw new TypeError("User-message processor must be a function");
  ...
  window.__vishrunRegisterPreGeneration = (handler) => {
    if (typeof handler !== "function")
      throw new TypeError("Pre-generation handler must be a function");
    const typedHandler = handler;
  ...
  const requestWorldInfoEntries = (requestId, signal) => {
    if (signal.aborted) {
  ...
  const runUserMessageHandlers = async (requestId, chatId, message, generationType) => {
      ...
      for (const handler of Array.from(userMessageHandlers)) {
        ...
        try {
          ...
          if (typeof result === "string") {
            ...
            if (result.cancelGeneration === true)
              cancelGeneration = true;
            if (result.removeMessage === true)
              removeMessage = true;
          }
          if (cancelGeneration)
            break;
  ...
  const runPreGenerationHandlers = async (requestId, chatId, generationType, activatedWorldInfo = []) => {
    ...
    const getWorldInfoEntries = () => {
      if (!worldInfoPromise)
        worldInfoPromise = requestWorldInfoEntries(requestId, controller.signal);
  ...
  const unsubBackendMsg = ctx.onBackendMessage((msg) => {
    ...
    if (m.type === "vsh_user_message_request" && m.requestId && m.chatId && m.message && typeof m.message.id === "string" && typeof m.message.content === "string") {
      runUserMessageHandlers(m.requestId, m.chatId, { id: m.message.id, content: m.message.content }, m.generationType);
      ...
      runPreGenerationHandlers(m.requestId, m.chatId, m.generationType, Array.isArray(m.activatedWorldInfo) ? m.activatedWorldInfo : []);
      ...
      if (m.error)
        ...
        pending.resolve(Array.isArray(m.entries) ? m.entries : []);
      ...
      if (pending.signal && pending.abortHandler)
        pending.signal.removeEventListener("abort", pending.abortHandler);
      const err = m.error?.startsWith("AbortError") ? new DOMException(m.error.replace(/^AbortError:?\s*/, "") || "Generation aborted", "AbortError") : new Error(m.error || "Generation failed");
      pending.reject(err);
    }
  });
  const unsubGenerationStopped = ctx.events.on("GENERATION_STOPPED", (payload) => {
    const stoppedChatId = payload?.chatId;
    for (const { controller, chatId } of preGenerationControllers.values()) {
      if (!stoppedChatId || !chatId || String(stoppedChatId) === String(chatId))
        controller.abort();
    }
  ...
  window.__vishrunGenerate = (opts, signalArg) => {
    const requestId = crypto.randomUUID();
    const raw = opts && typeof opts === "object" ? opts : {};
    const embeddedSignal = raw.signal;
  ...
  let scriptsPanel = createScriptsPanel(settingsMount, ctx, () => {
    reloadRunner();
  }, onReloadScript);
  initScriptStorage();
  ...
  async function loadFor(characterId) {
    console.log("[vishrun:diag] loadFor called:", characterId, "| inflight:", inflightCharacterId, "| lastLoaded:", lastLoadedCharacterId);
    if (!characterId) {
      clearActiveCard();
  ...
  const unsubChatChanged = ctx.events.on("CHAT_CHANGED", (payload) => {
    ...
    if (!pass)
      return;
    loadFor(p.characterId ?? ctx.getActiveChat().characterId ?? getActiveCard()?.characterId ?? null);
  });
  ...
  let lastPresetId = "__default__";
  let cancelPendingNavLoad = null;
  function scheduleDeferredLoadFor(expectedChatId) {
  ...
  return () => {
    ...
    for (const { controller } of preGenerationControllers.values())
      controller.abort();
    preGenerationControllers.clear();
    for (const { controller } of userMessageControllers.values())
      controller.abort();
    userMessageControllers.clear();
    for (const pending of pendingWorldInfoLookups.values())
      pending.reject(new DOMException("Vishrun disposed", "AbortError"));
